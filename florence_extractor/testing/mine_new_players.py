...
def poll_batches(csv_path):
        ...
        try:
            ...
            if hasattr(job, "done") and getattr(job, "done") is True:
                if job.state.name == "JOB_STATE_SUCCEEDED":
                    print(f"  Job {job_name} succeeded! Downloading results...")
                    
                    if not job.dest or not hasattr(job.dest, 'file_name') or not job.dest.file_name:
                        print("  Error: Cannot find output file URI in job destination.")
                        jobs_to_remove.append(job_name)
                        continue
                        
                    # Download the JSONL output file
                    output_bytes = client.files.download(file=job.dest.file_name)
                    ...
                    for line in output_text.strip().split('\\n'):
                        ...
                        try:
                            ...
                            cand = res_obj.get("response", {}).get("candidates", [])[0]
                            ...
                            print(f"  Failed to parse result for {req_name}: {e}")
                            data = None
                    ...
                    if new_rows:
                        ...
                        print(f"  Successfully appended {len(new_rows)} new rows to CSV!")
    ...
    for j in jobs_to_remove:
        ...
        try:
            ...
            player_unique_dir = os.path.dirname(state[j][first_req]["local_path"])
...
def main():
    ...
    # We use gemini-1.5-flash as the batch engine (it is extremely cheap and highly accurate)
    # Using 2.5 flash is also fine but 1.5 flash has robust batch support
