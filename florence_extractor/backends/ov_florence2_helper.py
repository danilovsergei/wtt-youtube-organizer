...
model_ids = ["microsoft/Florence-2-base-ft", "microsoft/Florence-2-base",
             "microsoft/Florence-2-large-ft", "microsoft/Florence-2-large"]
...
def fuse_cache_reorder(
    ...
    main_input_name = "decoder_input_ids" if model_has_input_output_name(
        ov_model, "decoder_input_ids") else "inputs_embeds"
    ...
    beam_idx.output(0).get_tensor().add_names(
        {"beam_idx"})  # why list is not accepted?
    ...
    for input_name in key_value_input_names:
        ...
        gather = opset13.gather(parameter_output_port,
                                beam_idx, opset13.constant(gather_dim))
...
def build_state_initializer(ov_model: ov.Model, batch_dim: int):
    ...
    batch = opset13.gather(opset13.shape_of(
        input_ids, output_type="i64"), opset13.constant([0]), opset13.constant(0))
    for op in ov_model.get_ops():
        if op.get_type_name() == "ReadValue":
            dims = [dim.min_length for dim in list(
                op.get_output_partial_shape(0))]
            dims[batch_dim] = batch
            dims = [opset13.constant(np.array([dim], dtype=np.int64)) if isinstance(
                dim, int) else dim for dim in dims]
            shape = opset13.concat(dims, axis=0)
            broadcast = opset13.broadcast(opset13.constant(
                0.0, dtype=op.get_output_element_type(0)), shape)
...
def get_consumer_nodes(node):
    consumer_inputs = set().union(
        *[output.get_target_inputs() for output in node.outputs()])
...
def find_output_nodes_of_dependent_subgraph(model: ov.Model, sources: list):
    ...
    other_inputs = set(model.get_parameters(
    ) + get_read_value_ops(model) + get_shape_of_ops(model)) - set(sources)
    ...
    edge_nodes = [
        node for node in nodes if get_consumer_nodes(node) & other_nodes]
    return edge_nodes
...
def patch_stateful_decoder(config, ov_model: ov.Model):
    ...
    not_kv_inputs = [input for input in ov_model.inputs if not any(
        name in key_value_input_names for name in input.get_names())]
    ...
    batch_dim = 1 if config.model_type == "chatglm" and not hasattr(
        config, "rope_ratio") else 0

    fuse_cache_reorder(ov_model, not_kv_inputs,
                       key_value_input_names, batch_dim)
    num_attention_heads = config.num_attention_heads if (
        config.model_type == "bloom" and is_transformers_version("<", "4.44")) else 1
    make_stateful(ov_model, not_kv_inputs, key_value_input_names,
                  key_value_output_names, batch_dim, num_attention_heads, None)
...
def patch_stateful_encoder_decoder(config, ov_model):
    ...
    insert_state_for_nodes(
        ...
        find_output_nodes_of_dependent_subgraph(
            ov_model, [ov_model.input("encoder_hidden_states").get_node()]),
    )
...
def convert_florence2(model_id, output_dir, orig_model_dir=None):
    ...
    processor = AutoProcessor.from_pretrained(
        orig_model_dir, trust_remote_code=True)
    ...
    if not (output_dir / TEXT_EMBEDING_NAME).exists():
        ...
        ov_model = ov.convert_model(
            text_embedding, example_input=example_input)
    ...
    if not (output_dir / ENCODER_NAME).exists():
        ...
        example_input = {"inputs_embeds": torch.zeros(
            [1, 590, model.config.text_config.d_model]), "attention_mask": torch.ones([1, 590])}
    ...
    if not (output_dir / DECODER_NAME).exists():
        ...
        for i in range(model.config.text_config.decoder_layers):
            decoder_key = torch.randn(
                [1, dec_attention_head, 1, dec_hidden_dim])
            decoder_value = torch.randn(
                [1, dec_attention_head, 1, dec_hidden_dim])
            encoder_key = torch.randn(
                [1, dec_attention_head, 590, enc_hidden_dim])
            encoder_value = torch.randn(
                [1, dec_attention_head, 590, enc_hidden_dim])
            ...
            output_names.extend(
                [f"present.decoder.{i}.key", f"present.{i}.decoder.value"])
...
class OVEncoder:
    ...
    def __init__(self, model_dir, parent_model, device, ov_config):
        ...
        self.input_names = {key.get_any_name(): idx for idx,
                            key in enumerate(self.model.inputs)}
        self.input_dtypes = {key.get_any_name(): key.get_element_type(
        ).get_type_name() for key in self.model.inputs}
        self.main_input_name = "input_ids"
        compiled_model = core.compile_model(
            self.model, self._device, ov_config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_ids is None and inputs_embeds is None:
            raise ValueError(
                "`input_ids` or `inputs_embeds` should be provided")
        ...
        # Run inference
        last_hidden_state = torch.from_numpy(self.request.infer(
            inputs, share_inputs=True, share_outputs=True)["last_hidden_state"]).to(self.device)
...
class OVDecoder:
    ...
    def __init__(self, model_path, parent_model, device, ov_config):
        ...
        self.output_dtypes = {key.get_any_name(): key.get_element_type(
        ).get_type_name() for key in self.model.outputs}
        ...
        # Initialize past length tracker
        self._past_length = 0
    ...
    def forward(
        ...
        if "beam_idx" in self.input_names:
            ...
            inputs["beam_idx"] = self.next_beam_idx if self.next_beam_idx is not None else np.arange(
        ...
        if not self.stateful:
            ...
            out_past_key_values = tuple(self.request.get_tensor(
    ...
    def _reorder_cache(self, past_key_values: tuple[tuple[torch.Tensor]], beam_idx: torch.Tensor) -> tuple[tuple[torch.Tensor]]:
        ...
        if self.stateful:
            ...
            for layer_past in past_key_values:
                ...
                reordered_past += (tuple(np.take(past_state, beam_idx, 0)
                                   for past_state in layer_past[:2]) + layer_past[2:],)
        return reordered_past


def validate_openvino_model(model_dir):
    """
    ...
    if missing_files:
        raise FileNotFoundError(
            f"OpenVINO model files not found in {model_dir}.\n"
            f"Missing: {', '.join(missing_files)}\n\n"
            f"Run conversion first:\n"
            f"  python florence_extractor/convert_to_openvino.py"
        )
...
class OVFlorence2Model:
    def __init__(self, model_dir, device, ov_config=None) -> None:
        ...
        validate_openvino_model(model_dir)
    ...
    def reset_state(self):
        """Reset decoder state for new inference."""
        if self.language_model.decoder.stateful:
            self.language_model.decoder.request.reset_state()
            self.language_model.decoder._past_length = 0
            self.language_model.decoder.next_beam_idx = None

    def generate(self, input_ids, inputs_embeds=None, pixel_values=None, **kwargs):
        # Reset state before each new generation to avoid state corruption
        self.reset_state()
    ...
    def _merge_input_ids_with_image_features(self, image_features, inputs_embeds):
        ...
        task_prefix_attention_mask = np.ones(
            (batch_size, task_prefix_embeds.shape[1]))
        ...
        inputs_embeds = np.concatenate(
            [image_features, task_prefix_embeds], axis=1)
...
class OVFlorence2LangModel(GenerationMixin):
    # Class attributes for transformers compatibility
    _is_stateful = False
    ...
    def __init__(self, model_dir, config, device, ov_config):
        ...
        self.decoder = OVDecoder(
            model_dir / DECODER_NAME, self, device, ov_config)
        self.decoder_with_past = OVDecoder(
            model_dir / DECODER_WITH_PAST_NAME, self, device, ov_config) if not self.decoder.stateful else None
